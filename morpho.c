...
void* erosion_thread(void*arguments){
	...
	/*//Coin haut gauche
	if((uint8)args.E_t[nrl][ncl] == 0 || E_t[nrl+1][ncl] == 0 || E_t[nrl][ncl+1] == 0 || E_t[nrl+1][ncl+1] == 0)
		E_t_bis[nrl][ncl] = 0;
	else
		E_t_bis[nrl][ncl] = E_t[nrl][ncl];
	//Coin haut droite
	if((uint8)args.E_t[nrl][nch] == 0 || E_t[nrl+1][nch] == 0 || E_t[nrl][nch-1] == 0 || E_t[nrl+1][nch-1] == 0)
		E_t_bis[nrl][nch] = 0;
	else
		E_t_bis[nrl][nch] = E_t[nrl][nch];
	//Coin bas droite
	if((uint8)args.E_t[nrh][nch] == 0 || E_t[nrh-1][nch] == 0 || E_t[nrh][nch-1] == 0 || E_t[nrh-1][nch-1] == 0)
		E_t_bis[nrh][nch] = 0;
	else
		E_t_bis[nrh][nch] = E_t[nrh][nch];
	//Coin bas gauche
	if((uint8)args.E_t[nrh][ncl] == 0 || E_t[nrh-1][ncl] == 0 || E_t[nrh-1][ncl+1] == 0 || E_t[nrh][ncl+1] == 0)
		E_t_bis[nrh][ncl] = 0;
	else
		E_t_bis[nrh][ncl] = E_t[nrh][ncl];
	//Bord gauche
	for(uint16_t i = nrl + 1; i < nrh; i++){
		if((uint8)args.E_t[i-1][ncl] == 0 || E_t[i-1][ncl+1] == 0 || E_t[i][ncl] == 0 || E_t[i][ncl+1] == 0 || E_t[i+1][ncl] == 0 || E_t[i+1][ncl+1] == 0)
			E_t_bis[i][ncl] = 0;
		else{
			E_t_bis[i][ncl] = E_t[i][ncl];
		}
	...
	//Rotation de variables
	int8_t p1 = (uint8)args->E_t[nrl][ncl],
	p2 = (uint8)args->E_t[nrl][ncl+1],
	p3 = (uint8)args->E_t[nrl][ncl+2],
	p4 = (uint8)args->E_t[nrl+1][ncl+2],
	p5 = (uint8)args->E_t[nrl+2][ncl+2],
	p6 = (uint8)args->E_t[nrl+2][ncl+1],
	p7 = (uint8)args->E_t[nrl+2][ncl],
	p8 = (uint8)args->E_t[nrl+1][ncl],
	p_curr = (uint8)args->E_t[nrl+1][ncl+1];

	for(uint16_t i = nrl + 1; i < nrh; i++){
		for(uint16_t j = ncl + 1; j < nch; j++){
			p3 = (uint8)args->E_t[i-1][j+1]; //Pixel haut droite
			p4 = (uint8)args->E_t[i][j+1]; //Pixel milieu droite
			p5 = (uint8)args->E_t[i+1][j+1]; //Pixel bas droite

			if(p1 == 0 || p2 == 0 ||p3 == 0 ||p4 == 0 ||p5 == 0 ||p6 == 0 ||p7== 0 ||p8== 0 ||p_curr== 0 ){
				E_t_bis[i][j] = 0;
			}
			else{
				E_t_bis[i][j]= (uint8)args->E_t[i][j];
			}
		...
		//Rotation de variables nouvelle ligne
		p1 = (uint8)args->E_t[i-1][ncl];
		p2 = (uint8)args->E_t[i-1][ncl+1];
		p3 = (uint8)args->E_t[i-1][ncl+2];
		p4 = (uint8)args->E_t[i][ncl+2];
		p5 = (uint8)args->E_t[i+1][ncl+2];
		p6 = (uint8)args->E_t[i+1][ncl+1];
		p7 = (uint8)args->E_t[i+1][ncl];
		p8 = (uint8)args->E_t[i][ncl];
		p_curr = (uint8)args->E_t[i][ncl+1];
	}
	return E_t_bis;
}
