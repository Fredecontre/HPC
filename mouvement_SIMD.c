	...
	for(uint16_t i = nrl; i <=nrh; i++){
		...
		for(uint16_t j = ncl; j <=nch; j++){
			...
			Reg_V_t_1 = (int8_t*)&V_t_1[i][j];
         	Reg_V_t = mipp::blend(Reg_V_t_1 + Reg_un, Reg_zero, Reg_V_t_1 < mipp::mul((mipp::Reg<int8_t>)Reg_N0,(mipp::Reg<int8_t>)Reg_O_t));
         	Reg_V_t= mipp::blend(Reg_V_t_1 - Reg_un, Reg_V_t_1, Reg_V_t_1 > mipp::mul((mipp::Reg<int8_t>)Reg_N0, (mipp::Reg<int8_t>)Reg_O_t));
